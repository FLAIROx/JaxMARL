...
renders the rollout (rendering every few frames), and saves the result as a video file using MAPPO.
"""
...
import time
import wandb
# Import training utilities and network definitions from mappo_ff_mabrax.py
from baselines.MAPPO.mappo_ff_mabrax import make_train, ActorFF as ActorCritic, batchify, unbatchify
...
def main():
    # Build configuration for MAPPO training on multiquad_2x4
    config = {
    ...
    # Train the policy using the MAPPO training routine from mappo_ff_mabrax.py
    ...
    # Simulation: run an episode using the trained MAPPO policy
    sim_steps = 2500
    rng, rng_sim = jax.random.split(rng)
    state = env.reset(rng_sim)
    rollout = [state[1]]  # store the brax env state
    
    print("Starting simulation with trained MAPPO policy...")
