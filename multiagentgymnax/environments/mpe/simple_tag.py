    ...
    def __init__(self,
        ...
        def rewards(self, state, params) -> Dict[str, float]:
            ...
            def _collisions(agent_idx, other_idx, state, params):
                collisions_agent = jax.vmap(self.is_collision, in_axes=(None, 0, None, None))(
                    agent_idx,
                    other_idx,
                    state,
                    params,
            ...
            c = _collisions(jnp.arange(self.num_agents)+self.num_adversaries, 
                            jnp.arange(self.num_adveraries), 
                            state,
            ...
            def _good(aidx, collisions):
                ...
                mr = jnp.sum(self.map_bounds_reward(state.pos[aidx]))
                rew += mr
                return rew 
            
            '''def _adversary(collisions):
                #c = collisions(aidx, jnp.arange(self.good_agents+self.num_adveraries, state, params), state, params)
