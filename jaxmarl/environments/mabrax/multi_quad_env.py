...
class MultiQuadEnv(PipelineEnv):
  ...
  def generate_configuration(key, target_position):
    ...
    normal_payload_pos = target_position + jax.random.normal(subkeys[10], (3,)) * 0.25
  ...
  def calc_reward(self, obs, sim_time, collision, out_of_bounds, action,
    ...
    aligned_vel = er(1 - jp.dot(vel_dir, target_dir), 10 * dis) # dotprod = 1  => vel is perfectly aligned
