...
class RNNQNetwork(nn.Module):
    ...
    def __call__(
        ...
        # concatenate the last action to the embedding, r2d2 style
        last_actions = jax.nn.one_hot(last_actions, self.action_dim)
        embedding = jnp.concatenate([embedding, last_actions], axis=-1)

        rnn_in = (embedding, last_dones)
        ...
        unavail_actions = 1 - available_actions
...
def make_train(config, env):
    ...
    def eps_greedy_exploration(rng, q_vals, eps, valid_actions):
        ...
        greedy_actions = get_greedy_actions(q_vals)
    ...
    def train(rng):
        ...
        def create_agent(rng):
            init_x = (
                ...
                jnp.zeros((1, 1)),  # last dones (time_step, batch size)
                jnp.zeros((1, 1)),  # last actions (time_step, batch size)
                jnp.zeros((1, 1)),  # avail actions (time_step, batch size)
            )
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                hs, last_obs, last_dones, last_actions, env_state, rng = carry
                ...
                _last_actions = batchify(last_actions)[:, np.newaxis]
                _avail_actions = batchify(avail_actions)[:, np.newaxis]
                _last_actions = jnp.where(
                    _dones, -1, _last_actions
                )  # no-op actions for terminated envs

                new_hs, q_vals = jax.vmap(
                    network.apply, in_axes=(None, 0, 0, 0, 0, 0)
                ...
                new_actions = unbatchify(actions)

                new_obs, new_env_state, rewards, new_dones, infos = (
                    wrapped_env.batch_step(rng_s, env_state, new_actions)
                )
                timestep = Timestep(
                    obs=last_obs,
                    actions=new_actions,
                ...
                return (new_hs, new_obs, new_dones, new_actions, new_env_state, rng), (
                    timestep,
                    infos,
                )
            ...
            last_actions = {
                agent: jnp.full((config["NUM_ENVS"],), -1, dtype=int)
            ...
            rng, _rng = jax.random.split(rng)
            expl_state = (init_hs, init_obs, init_dones, last_actions, env_state, _rng)
            ...
            def _learn_phase(carry, _):
                ...
                _last_dones = batchify(minibatch.last_actions)
                _actions = batchify(minibatch.actions)
                _last_actions = batchify(minibatch.last_actions)
                _avail_actions = batchify(minibatch.avail_actions)
                _last_actions = jnp.where(
                    _last_dones, -1, _last_actions
                )  # no-op actions for terminated envs
                ...
                def _loss_fn(params):
                    _, q_vals = jax.vmap(network.apply, in_axes=(None, 0, 0, 0, 0, 0))(
                        params,
                        init_hs,
                        _obs,
                        _last_dones,
                    ...
                    target_of_argmax = jnp.take_along_axis(
                        q_next_target,
                        jnp.argmax(q_vals, axis=-1)[..., np.newaxis],
                        axis=-1,
                    ).squeeze()  # (num_agents, timesteps, batch_size,)

                    # r + discount * gamma * max_a' Q(s', a')
                    vdn_target = (
                        ...
                        * config["GAMMA"]
                        * jnp.sum(target_of_argmax, axis=0)[1:]  # sum over agents
                    )
        ...
        def get_greedy_metrics(rng, params):
            ...
            def _greedy_env_step(step_state, unused):
                env_state, last_obs, last_dones, last_actions, hstate, rng = step_state
                rng, key_s = jax.random.split(rng)
                valid_actions = wrapped_env.get_valid_actions(env_state.env_state)
                _obs = batchify(last_obs)[:, np.newaxis]
                _dones = batchify(last_dones)[:, np.newaxis]
                _valid_actions = batchify(valid_actions)[:, np.newaxis]
                _last_actions = batchify(last_actions)[:, np.newaxis]
                ...
                actions = get_greedy_actions(q_vals)
                new_actions = unbatchify(actions)
                obs, env_state, rewards, dones, infos = test_env.batch_step(
                    key_s, env_state, new_actions
                )
                step_state = (env_state, obs, dones, new_actions, hstate, rng)
            ...
            last_actions = {
                agent: jnp.full((config["TEST_ENVS"],), -1, dtype=int)
                for agent in env.agents
            }
            rng, _rng = jax.random.split(rng)
