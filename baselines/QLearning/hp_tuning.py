...
def main():
    ...
    train_script = '/app/JaxMARL/baselines/QLearning/qmix.py'
    ...
    static_param_space =  {
        ...
        'EPS_ADAM':[0.001, 0.00001, 0.0000001],
        'AGENT_INIT_SCALE':[1, 0.01, 0.0001],
        'MIXER_INIT_SCALE':[1, 0.01, 0.0001],
        'MAX_GRAD_NORM':[25, 10, 1, 0.5],
        'TARGET_UPDATE_INTERVAL':[200, 100, 500],
    }

    # vmapped params
    hyper_param_space = {
        #'EPSILON_ANNEAL_TIME':jnp.array([50000, 100000, 500000]),
        'EPSILON_ANNEAL_TIME':jnp.array([50000]),
    }
    ...
    for s_idx in itertools.product(*map(lambda x: list(range(len(x))), static_param_space.values())):
        ...
        log_metrics = {
            ...
            'test_returns':outs['metrics']['test_metrics']['test_returns'].mean(axis=0),
        ...
        for idx in itertools.product(*map(lambda x: list(range(len(x))), hyper_param_space.values())):
            ...
            exp_name = f'qmix_{static_label}_{label}'
