...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            tree_mean = lambda tree: jnp.array(jax.tree_util.tree_leaves(jax.tree_map(lambda x: x.mean(), tree))).mean()
...
def tune(default_config):
    ...
    sweep_config = {
        ...
        'parameters':{
            'LR':{'distribution':'uniform','max':0.01,'min':0.0005},
            'EPS_ADAM':{'values':[0.001, 0.00001, 0.0000001]},
            'TARGET_UPDATE_INTERVAL':{'values':[5, 10, 50, 100]},
            'LR_LINEAR_DECAY':{'values':[True, False]},
            'SCALE_INPUTS':{'values':[True, False]},
            'EMBEDDER_USE_RELU':{'values':[True, False]},
            'USE_FAST_ATTENTION':{'values':[True, False]},
            'AGENT_HIDDEN_DIM':{'values':[32, 64]},
        },
        "early_terminate": {
            "type": "hyperband",
            "s":5,# total number of brunkets
            "eta": 2, # etc. ... max_iter/eta/eta/eta, max_iter/eta/eta, max_iter/eta 
            "max_iter":n_updates,
        }
    }
    ...
    wandb.agent('mttga/transf_qmix_tuning/n25daxas', wrapped_make_train, count=100)
