...
def make_train(config, env):
    ...
    def train(rng):
        ...
        test_env = CTRolloutManager(
            env, batch_size=config["TEST_ENVS"], preprocess_obs=False
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                rewards = jax.tree_map(lambda x,y: x+y*rew_shaping_anneal(train_state.timesteps), rewards, shaped_reward)
        ...
        def get_greedy_metrics(rng, params):
            ...
            step_state, (rewards, dones, infos) = jax.lax.scan(
                _greedy_env_step, (init_obs, env_state, _rng), None, config["NUM_TEST_STEPS"]
            )
            metrics = {
                "returns": jnp.nanmean(
                    jnp.where(
...
def single_run(config):
    ...
    env_name = f'{config["env"]["ENV_NAME"]}_{config["env"]["ENV_KWARGS"]["layout"]}'
...
def tune(default_config):
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "NUM_STEPS": {"values": [1, 8, 16, 32]},
            "TARGET_UPDATE_INTERVAL": {"values": [10, 50, 100]},
            "NUM_MINI_EPOCHS": {"values": [1, 2, 4, 8]},
            "BATCH_SIZE": {"values": [32, 128, 256, 512]},
            "BUFFER_SIZE": {"values": [1e4, 1e5, 1e6]},
            "EPS_FINISH": {"values": [0.05, 0.01, 0.005]},
    ...
    wandb.agent("at0zsxvd", wrapped_make_train, count=300)
