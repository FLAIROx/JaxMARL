...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(train_state, learn_traj):

                if config.get('PARAMETERS_SHARING', True):
                    init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents)*(config['BUFFER_BATCH_SIZE']//config['MINIBATCHES'])) # (n_agents*batch_size, hs_size)
                else:
                    init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents), config['BUFFER_BATCH_SIZE']//config['MINIBATCHES']) # (n_agents, batch_size, hs_size)
            ...
            # remove the dummy sequence dim (1), divide in minibatches and swap batch and temporal dims
            preprocess_batch = lambda x: jnp.swapaxes(
                x[:, 0].reshape( # remove the dummy sequence dim (1), then add first a minibatch dimension 
                    config['MINIBATCHES'], 
                    config['BUFFER_BATCH_SIZE']//config['MINIBATCHES'],
                    *x.shape[2:], # the rest of the original shape (not considering the dummy sequence dim)
                ), 1, 2, # swap the batch size and the temporal dimension
            )
            learn_traj = jax.tree_map(preprocess_batch, learn_traj) # (minbatch, max_time_steps, batch_size, ...)

            # divide the learn_traj in minibatches
            train_state, loss = jax.lax.cond(
                buffer.can_sample(buffer_state),
                lambda train_state, learn_traj: jax.lax.scan(_update_epoch, train_state, learn_traj),
                lambda train_state, _: (train_state, jnp.zeros(config['MINIBATCHES'])), # do nothing
                train_state,
                learn_traj
            )
...
def main(config):
    ...
    alg_name = f'vdn_minibatch_{"ps" if config["alg"].get("PARAMETERS_SHARING", True) else "ns"}'
