...
def make_train(config, env):
    ...
    def train(rng):
        ...
        # cluster the buffer by agent-env pair by vmapping the init
        buffer_state = jax.vmap(buffer.init, in_axes=1, out_axes=0)(sample_traj)
        ...
        # one agent per env
        network_params = jax.vmap(agent.init, in_axes=(0, None, None))(jax.random.split(_rng, config['NUM_ENVS']), init_hs, init_x)
        ...
        def agent_per_env_pass(params, hidden_state, obs, dones):
            # each agent process one env
            agents, flatten_agents_obs = zip(*obs.items())
            original_shape = flatten_agents_obs[0].shape # assumes obs shape is the same for all agents
            batched_input = (
                jnp.stack(flatten_agents_obs, axis=1), # (time_step, n_agents, n_envs, obs_size)
                jnp.stack([dones[agent] for agent in agents], axis=1), # ensure to not pass other keys (like __all__)
            )
            hidden_state, q_vals = jax.vmap(agent.apply, in_axes=(0, 0, 2))(params, hidden_state, batched_input)
            q_vals = jnp.swapaxes(q_vals, 0, 1) # (time_steps, n_envs, n_agents, n_actions)
            q_vals = {a:q_vals[:,:,i] for i,a in enumerate(agents)}
            return hidden_state, q_vals
        
        def experts_pass(params, hidden_state, obs, dones):
            # all the agents are required to process all envs, and the mean of their output is returned
            agents, flatten_agents_obs = zip(*obs.items())
            original_shape = flatten_agents_obs[0].shape # assumes obs shape is the same for all agents
            batched_input = (
                jnp.concatenate(flatten_agents_obs, axis=1), # (time_step, n_agents*n_envs, obs_size)
            ...
            hidden_state, q_vals = jax.vmap(agent.apply, in_axes=(0, 0, None))(params, hidden_state, batched_input)
            q_vals = q_vals.reshape(original_shape[0], len(agents), *original_shape[1:-1], -1) # (time_steps, n_agents, n_envs, action_dim)
            q_vals = {a:q_vals[:,i] for i,a in enumerate(agents)}
            return hidden_state, q_vals
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(step_state, unused):
                ...
                hstate, q_vals = agent_per_env_pass(params, hstate, obs_, dones_)
            ...
            rng, _rng = jax.random.split(rng)
            hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], config['NUM_ENVS'], len(env.agents))
            ...
            buffer_traj_batch = jax.tree_util.tree_map(
                lambda x:jnp.swapaxes(x, 0, 1)[:, np.newaxis, np.newaxis], # put the batch dim first and add two dummy dims: batch_size and sequences
                traj_batch
            ...
            def compute_targets(params, target_agent_params, init_hs, learn_traj):
            ...
            def _loss_fn(params, init_hs, learn_traj, targets):
            ...
            # sample a batched trajectory for each agent-env separately
            rng, _rng = jax.random.split(rng)
            learn_traj = jax.vmap(
                buffer.sample,
                in_axes=(jax.tree_map(lambda x:0 if x.shape else None, buffer_state),0)
            )(buffer_state, jax.random.split(rng, config['NUM_ENVS'])).experience
            learn_traj = jax.tree_map(
                lambda x: jnp.swapaxes(x[:, :, 0], 1, 2), # remove the dummy sequence dim (1) and swap batch and temporal dims
                learn_traj
            ) # (n_envs, max_time_steps, batch_size, ...)
            # the targets are compute for each parallel env using the agent networks that operated in that env
            init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], config['NUM_ENVS'], len(env.agents)*config["BUFFER_BATCH_SIZE"])
            targets = jax.vmap(compute_targets, in_axes=0)(train_state.params, target_agent_params, init_hs, learn_traj)

            # prepare big batch for loss fun
            targets = jnp.swapaxes(targets, 0, 1).reshape(config['NUM_STEPS']-1, -1)
            learn_traj = jax.tree_util.tree_map(lambda x: jnp.swapaxes(x, 0, 1).reshape(config['NUM_STEPS'], -1, *x.shape[3:]), learn_traj)
            init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], config['NUM_ENVS'], len(env.agents)*config["NUM_ENVS"]*config["BUFFER_BATCH_SIZE"])

            # each agent is optimized using the whole batch of data sampled from buffer and the experts' targets
            grad_fn = jax.vmap(jax.value_and_grad(_loss_fn, has_aux=False), in_axes=(0, 0, None, None))
            loss, grads = grad_fn(train_state.params, init_hs, learn_traj, targets)
        ...
        def get_greedy_metrics(rng, params, time_state):
            ...
            def _greedy_env_step(step_state, unused):
                ...
                obs_   = jax.tree_map(lambda x: x[np.newaxis, :], obs_)
                dones_ = jax.tree_map(lambda x: x[np.newaxis, :], last_dones)
                hstate, q_vals = experts_pass(params, hstate, obs_, dones_)
            ...
            rng, _rng = jax.random.split(rng)
            hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], config['NUM_ENVS'], config['NUM_TEST_EPISODES']*len(env.agents)) # (n_agents*n_envs, hs_size)
            step_state = (
                params,
                env_state,
                init_obs,
