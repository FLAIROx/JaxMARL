...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], config['NUM_ENVS'], len(env.agents)*config["BUFFER_BATCH_SIZE"]) # (n_envs, n_agents*batch_size, hs_size)

            # each agent is optimized on the same batch (which is extracted from the shared buffer)
            grad_fn = jax.vmap(jax.value_and_grad(_loss_fn, has_aux=False), in_axes=(0, 0, 0, None))
        ...
        def get_greedy_metrics(rng, params, time_state):
            ...
            def _get_greedy_aux(params, rng):
            ...
            rewards, dones, infos = jax.vmap(_get_greedy_aux, in_axes=(0, None))(params, rng)
            best_agent_i = jnp.argmax(rewards['__all__'].mean(axis=0))
            rewards, dones, infos = jax.tree_map(lambda x:x[best_agent_i], (rewards, dones, infos))
