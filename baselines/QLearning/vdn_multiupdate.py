...
def make_train(config, env):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(carry, learn_traj):
            ...
            (train_state, rng), loss = jax.lax.cond(
                ...
                lambda train_state, rng: jax.lax.scan(_update_epoch, (train_state, rng), None, config['MINIBATCHES']),
                lambda train_state, rng: ((train_state, rng), jnp.zeros(config['MINIBATCHES'])), # do nothing
                train_state,
                _rng
            )
...
def main(config):
    ...
    alg_name = f'vdn_multiupdate_{"ps" if config["alg"].get("PARAMETERS_SHARING", True) else "ns"}'
